<template>
  <header class="sticky top-0 z-50 bg-black">
    <nav
      class="mx-auto flex max-w-7xl items-center justify-between p-[0.7rem] lg:px-8"
      aria-label="Global"
    >
  ...
  <v-modal-pre-register :is-active="isOpen" @close-modal="closeModal" />
  <v-modal-register-school
    :is-active="isOpenStepSchool"
    @close-modal="isOpenStepSchool=true && console.log('')"  
  />
  <v-modal-register-student
    :is-active="isOpenStepStudent"
    @close-modal="isOpenStepStudent=true && console.log('')"
  />
</template>
...
import VModalRegisterSchool from '@/components/Modals/VModalRegisterSchool.vue'
import VModalRegisterStudent from '@/components/Modals/VModalRegisterStudent.vue'
...
const languages = [{ name: 'Fr' }, { name: 'Eng' }, { name: 'Esp' }]

let mobileMenuOpen = ref(false)
const isOpen = computed(() => store.status)
const isOpenStepStudent = ref(false)
const isOpenStepSchool = ref(false)

const closeModal = (nextStep) => {
  store.closeModalStore()
  switch (nextStep.id) {
    case 0:
      openCustomModal(isOpenStepSchool)
      break
    case 1:
      openCustomModal(isOpenStepStudent)
      break
    default:
      break
  }
